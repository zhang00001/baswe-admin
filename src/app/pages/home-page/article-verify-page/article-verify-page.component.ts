...
@Component({
  selector: "app-article-verify-page",
  templateUrl: "./article-verify-page.component.html",
  styleUrls: ["./article-verify-page.component.css"]
})
export class ArticleVerifyPageComponent implements OnInit {
  editor: EditorClass;
  ViewState = ViewState;
  ...
  async initEditor(content_md?: string) {
    ...
    setTimeout(async () => {
      ...
      setTimeout(() => this.editor.previewing(), 500);
    }, 1000);
  }
  ...
  async refuseArticle(articles_id) {
    await this.admin.refuseArticle(articles_id);
    await this.getArticlePage();
  }
  async passArticle(articles_id) {
    await this.admin.passArticle(articles_id);
    await this.getArticlePage();
  }
}
